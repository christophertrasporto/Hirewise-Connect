...
import type { RoleKey } from "../src/server/policies/permissions";
import { AGREEMENTS, SKILLS, SOFTWARE, assertDemoSeedAllowed, checksum, resolveSeedScope, seedFoundation } from "./seed-foundation";

/**
 * Deterministic seed (Section 12). Safe to re-run: every write is an upsert.
 *   npm run db:seed              foundation reference data plus demo accounts (local development, CI, e2e)
 *   npm run db:seed:foundation   reference data only; no users (production and staging)
 * Demo passwords are for local development only; the demo scope refuses Supabase hosts.
 */
...
async function main() {
  ...
  const { roleIds, permIds, agreementIds, skillIds, softwareIds, labelIds, templateIds } = await seedFoundation(prisma);
  if (scope === "foundation") {
    console.log("Foundation seed complete: roles, permissions, agreements (LEGAL_PLACEHOLDER), settings, taxonomies, Academy labels and templates, verification ladder, deposit policies.");
    console.log("No user accounts were created. Create the first Super Admin with: npm run bootstrap:admin -- --email <you@yourdomain>");
    return;
  }
  ...
  // Phase 3 — Academy demo courses with exams.
  ...
  // Phase 4 — Commercial demo: published client rates, compensation, an active placement with a paid deposit.
